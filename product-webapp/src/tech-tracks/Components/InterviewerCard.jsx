...
const InterviewerCard = (props) => {
  ...
  const postData = (data, description) => {
    axios
      .post("http://18.222.217.10:8080/tagservice/api/v1/bookedSlot", {
        // id: data.id,
      ...
      .then((response) => {
        if(response.status===201){
          axios.put(`http://18.222.217.10:8080/interviewerservice/api/v1/updateSlot`,{
            slotId: data.slotId,
  ...
  const ShowSlotsModal = (props) => {
    ...
    const fetchData = () => {
      ...
      axios
        .get(
          `http://18.222.217.10:8080/interviewerservice/api/v1/availableSlot/${modalShow.email}`
        )
        .then((res) => {
          return res.data;
        })
