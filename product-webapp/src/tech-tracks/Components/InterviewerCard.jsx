...
const InterviewerCard = (props) => {
  ...
  const postData = (data, description) => {
    axios
      .post("https://18.222.217.10:8080tagservice/api/v1/bookedSlot", {
        // id: data.id,
      ...
      .then((response) => {
        if(response.status===201){
          axios.put(`https://18.222.217.10:8080interviewerservice/api/v1/updateSlot`,{
            slotId: data.slotId,
  ...
  const ShowSlotsModal = (props) => {
    ...
    const fetchData = () => {
      ...
      axios
        .get(
          `https://18.222.217.10:8080interviewerservice/api/v1/availableSlot/${modalShow.email}`
        )
        .then((res) => {
          return res.data;
        })
